...
CREATE TABLE `books` (
  ...
  `bookDescription` mediumtext,
  `bookPicture` varchar(3000) DEFAULT NULL,
  `bookFile` mediumblob,
  `bookStatus` tinyint(4) NOT NULL,
  PRIMARY KEY (`idBooks`),
  KEY `fk_Books_Authors_idx` (`idAuthors`),
  KEY `fk_Books_Genre1_idx` (`idGenre`),
  CONSTRAINT `fk_Books_Authors` FOREIGN KEY (`idAuthors`) REFERENCES `authors` (`idAuthors`) ON UPDATE RESTRICT,
  CONSTRAINT `fk_Books_Genre1` FOREIGN KEY (`idGenre`) REFERENCES `genre` (`idGenre`) ON DELETE RESTRICT ON UPDATE RESTRICT
...
-- Dump completed on 2019-07-03 14:05:35
